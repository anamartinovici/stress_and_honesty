...
f_plot_interaction <- function(choice_fit, choice_pars, grid_std_cheating) {
	choice_draws <- rstan::extract(choice_fit, pars = choice_pars)
	choice_draws <- data.frame(choice_draws)
	
	aux_list <- NULL
	index_list <- 1
	for(cheat_value in grid_std_cheating) {
		aux_df <- choice_draws %>%
			mutate(u_cheat_control = gamma_00 + gamma_std_cheat * cheat_value,
				   u_cheat_stress  = gamma_00 + gamma_std_cheat * cheat_value +
				   	gamma_stress + gamma_std_cheat_stress*cheat_value)
		aux_df <- aux_df %>%
		...
		aux_list[[index_list]] <- aux_df %>%
			...
			mutate(cheat_value = cheat_value)
		index_list <- index_list + 1
	}
	df_plot <- map_dfr(aux_list, rbind)
	
	df_plot <- df_plot %>%
		pivot_longer(-cheat_value,
	...
	df_plot %>%
		ggplot(aes(x = cheat_value, group = experimental_group)) +
		geom_ribbon(aes(ymin = post_LCI, ymax = post_UCI, fill = experimental_group),
					alpha = 0.35) +
		geom_line(aes(y = post_mean, color = experimental_group)) + 
		theme_bw() +
		ylab("Probability of cheating at t=2") +
		xlab("Standardized cheating at t=1") + 
...
f_prep_stan_data <- function(part_info, part_and_task_info,
							 exclusion_C1, exclusion_C2) {
	
	if(!(exclusion_C1 %in% c("no_exclusion", "Hand_out"))) {
		stop("the exclusion criteria 1 is not specified")
	}
	
	if(!(exclusion_C2 %in% c("no_exclusion", "top_bottom_5", "top_bottom_10", "top_bottom_15"))) {
